...
## Generated crowd LOD build

Production crowd rendering no longer loads the master character GLBs directly. `npm run build:crowd` derives 50 crowd-only GLBs under `public/generated/crowd/`; `prebuild` and `predev` run this automatically. Master files under `public/model/characters/` remain read-only.

The generated pass preserves the skinned hierarchy and distinctive mesh/material content, removes animation clips unused by the cheering pose, runs safe `prune()` / `dedup()`, and downsizes embedded PNG/JPEG/WebP textures to a maximum 256×256 while retaining their original image format. It deliberately avoids Draco, Meshopt and KTX2 so the runtime needs no new decoder configuration.

Generated assets are budgeted independently from the source audit: 2 MiB max per LOD, 8 MiB for the first 10 start-critical LODs, and 50 MiB for all 50. The generated `manifest.json` records source/generated bytes and per-character reduction. These thresholds are build failures, not advisory warnings.
