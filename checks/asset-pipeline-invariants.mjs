...
const pkg=JSON.parse(fs.readFileSync('package.json','utf8')),skier=fs.readFileSync('src/skier.js','utf8'),opt=fs.readFileSync('scripts/assets-optimize.mjs','utf8'),audit=fs.readFileSync('scripts/lib/asset-audit.mjs','utf8');
...
assert(opt.includes('assertCompatible'));assert(opt.includes('ASSET_MIN_SAVINGS_PCT'));assert(opt.includes("textures:'preserved; no automatic lossy resize/recompression'"));
