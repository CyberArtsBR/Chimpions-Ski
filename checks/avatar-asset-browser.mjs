...
try{
 for(const entry of avatars){
  for(const mode of ['ski','snowboard']){
   const page=await context.newPage(),errors=[],failed=[],requestedGlbs=[];
   page.on('request',r=>{if(/^https?:/i.test(r.url())&&/\.glb(?:[?#]|$)/i.test(r.url()))requestedGlbs.push(r.url());});
   ...
   assert.equal(requestedGlbs.length,0,entry.name+' fresh boot must not request a GLB');
   await page.locator('#choose').evaluate(el=>el.click());
   ...
   assert.equal(new Set(requestedGlbs).size,1,entry.name+' selection should request only the selected built-in GLB');
 ...
 const localPage=await context.newPage(),localErrors=[],localFailed=[],localRequestedGlbs=[];
 localPage.on('request',r=>{if(/^https?:/i.test(r.url())&&/\.glb(?:[?#]|$)/i.test(r.url()))localRequestedGlbs.push(r.url());});
 ...
 assert.equal(localRequestedGlbs.length,0,'local GLB fresh boot must not request a GLB');
 await localPage.locator('#choose').evaluate(el=>el.click());
 ...
 assert.equal(localRequestedGlbs.length,0,'local GLB must not create remote GLB traffic');
