...
assert.equal(QUALITY_PROFILES.max.shadows,false,'MAX must not use realtime shadow maps');
assert.equal(QUALITY_PROFILES.max.shadowMapSize,0,'MAX should not allocate shadow maps');
assert(QUALITY_PROFILES.max.shadowRadius<=24,'MAX shadow coverage is no longer near-field');
assert.equal(QUALITY_PROFILES.max.shadowUpdateHz,0,'MAX should not schedule realtime shadow updates');
