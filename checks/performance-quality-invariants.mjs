...
assert.equal(QUALITY_PROFILES.high.shadows,false,'HIGH must not pay for realtime shadow maps');
assert.equal(QUALITY_PROFILES.max.shadows,true,'MAX must enable bounded near-field shadows');
assert(QUALITY_PROFILES.max.shadowMapSize<=2048,'MAX shadow map budget exceeded');
assert(QUALITY_PROFILES.max.shadowRadius<=24,'MAX shadow coverage is no longer near-field');
assert(QUALITY_PROFILES.max.shadowUpdateHz>0&&QUALITY_PROFILES.max.shadowUpdateHz<=30,'MAX shadow updates must be throttled');
assert.equal(QUALITY_PROFILES.high.contactGrounding,true,'HIGH should keep the cheap grounding pass');
assert.equal(QUALITY_PROFILES.medium.contactGrounding,false,'MEDIUM inherited premium grounding cost');
assert.equal(QUALITY_PROFILES.low.contactGrounding,false,'LOW inherited premium grounding cost');
