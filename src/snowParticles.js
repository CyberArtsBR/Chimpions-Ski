...
#include <colorspace_fragment>}`});
  ...
  function spray(dt,x,y,z,speed,edge,air,landingPulse,running=true,rideMode='ski'){
    ...
    if(!air&&carve>.12){emitBudget+=dt*(8+speed01*28)*carve*densityScale*(board?1.4:1);while(emitBudget>=1){emitBudget-=1;const r=rand(serial+17),r2=rand(serial+31),dir=Math.sign(edge||1),width=board?.78:.46;spawn(x+(r-.5)*width-dir*(board?.16:.10),y+.035+r2*.075,z+.32+(r2-.5)*.18,dir*(board?2.25:1.65)*(carve+.18)+(r-.5)*.35,.35+r2*(.65+carve*.65),.28+speed01*.72+r*.28,3.1+r2*4.1+(board?1.1:0),.38+r*.34,.10+carve*.20+speed01*.06);}}else emitBudget=Math.min(emitBudget,.6);
