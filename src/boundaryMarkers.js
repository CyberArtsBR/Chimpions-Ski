...
const snowCapGeometry=new THREE.CylinderGeometry(.195,.185,.052,16);
...
export function createBoundaryMarkers({world,terrainHeight,limit=COURSE_FLAG_X,countPerSide=40,spacing=7.2,woodTexture=null,decorativeShadows=true}){
  ...
  const snowMaterial=new THREE.MeshPhysicalMaterial({color:0xffffff,roughness:.84,clearcoat:.05,clearcoatRoughness:.62,sheen:.22,sheenColor:new THREE.Color(0xffffff)});
  const iron=new THREE.MeshStandardMaterial({color:0x6d8794,roughness:.29,metalness:.88});
  const ledCoreMaterial=new THREE.MeshBasicMaterial({color:new THREE.Color().setRGB(.22,1.55,6.8),toneMapped:false});
  const ledGlowMaterial=new THREE.MeshBasicMaterial({color:new THREE.Color().setRGB(.07,.62,3.15),transparent:true,opacity:.30,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending});
  const n=countPerSide*2;
  const posts=new THREE.InstancedMesh(postGeometry,postMaterial,n),caps=new THREE.InstancedMesh(capGeometry,capMaterial,n),snowCaps=new THREE.InstancedMesh(snowCapGeometry,snowMaterial,n),feet=new THREE.InstancedMesh(footGeometry,capMaterial,n);
  ...
  const plates=new THREE.InstancedMesh(plateGeometry,iron,n*2),bolts=new THREE.InstancedMesh(boltGeometry,iron,n*4);
  const meshes=[posts,caps,snowCaps,feet,rails,ledRails,ledGlows,postLeds,postGlows,plates,bolts];
  ...
  function refresh(){for(let i=0;i<countPerSide;i++)for(let sideIndex=0;sideIndex<2;sideIndex++){const side=sideIndex===0?-1:1,idx=sideIndex*countPerSide+i,z=positions[i],x=side*(limit+.28),ground=terrainHeight(x,z-travel),lean=(hash(idx+19)-.5)*.020,scale=.98+hash(idx+61)*.04;
    put(posts,idx,x,ground+.95*scale,z,0,0,lean,1,scale,1);put(caps,idx,x-Math.sin(lean)*1.90*scale,ground+1.94*scale,z,0,0,lean);put(snowCaps,idx,x-Math.sin(lean)*1.95*scale,ground+1.988*scale,z,0,0,lean,1.02,1,1.02);put(feet,idx,x,ground+.075,z);
    const innerPostX=x-side*.19;put(postGlows,idx,innerPostX,ground+1.06*scale,z-side*.005,0,0,lean,1,scale,1);put(postLeds,idx,innerPostX-side*.008,ground+1.06*scale,z-side*.008,0,0,lean,1,scale,1);
    const railZ=z-spacing*.5,farGround=terrainHeight(x,z-spacing-travel),angle=Math.atan2(farGround-ground,spacing);
    for(let level=0;level<2;level++){const h=.73+level*.57,railIndex=idx*2+level,railLength=Math.hypot(spacing,farGround-ground)+.22;put(rails,railIndex,x,ground+(farGround-ground)*.5+h,railZ,angle,0,0,1,1,railLength);const innerX=x-side*.171;put(ledGlows,railIndex,innerX,ground+(farGround-ground)*.5+h,railZ,angle,0,0,1,1,railLength*.987);put(ledRails,railIndex,innerX-side*.009,ground+(farGround-ground)*.5+h,railZ,angle,0,0,1,1,railLength*.982);put(plates,railIndex,x-side*.157,ground+h,z);for(let b=0;b<2;b++)put(bolts,idx*4+level*2+b,x-side*.18,ground+h+(b?1:-1)*.087,z,0,0,0,.4,1,1);}}
    for(const mesh of meshes)mesh.instanceMatrix.needsUpdate=true;
  }
  function reset(){travel=0;pulseTime=0;for(let i=0;i<countPerSide;i++)positions[i]=-8-i*spacing;refresh();}
  function update(dt,speed){pulseTime+=Math.max(0,Number(dt)||0);ledGlowMaterial.opacity=.275+Math.sin(pulseTime*2.1)*.035;if(!speed)return;const dz=speed*dt;travel+=dz;for(let i=0;i<countPerSide;i++){positions[i]+=dz;while(positions[i]>18)positions[i]-=countPerSide*spacing;}refresh();}
